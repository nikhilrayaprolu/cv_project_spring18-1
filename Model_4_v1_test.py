...
f_mean = np.array(([73.8450, 70.7520, 68.37821])).reshape([1,1,3])
...
def load_path(alpha, eps, hard_mode = False):
	...
	return np.array(alphas_abspath),np.array(epses_abspath)
...
def load_data(batch_alpha_paths, batch_eps_paths, fg):
	...
	while batch < batch_size:
		...
		f_g = misc.imread(fg + '/'+batch_alpha_paths[i].split('/')[-1]).astype(np.float32)
...
def preprocessing_single(alpha, eps, f_g, name, image_size=320):
	...
	#print(trimap.shape, alpha.shape, eps.shape)
	train_pre = np.concatenate([trimap, alpha, eps, f_g],2)
	if crop_size == 320:
...
class MattingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        batch_RGBs, batch_trimaps, batch_alphas, batch_FGs = load_data(batch_alpha_paths, batch_merged_paths, fg)
...
def load_alphamatting_path(test_alpha):
    ...
    print(len(images), len(fg_images), len(tri_images))
...
def load_alphamatting_data(rgb_path, trimap_path, fg_path):
    ...
    fg = misc.imread(fg_path)[:,:,:3]
    print(fg_path, fg.shape)
...
paths_alpha, paths_merged = load_path(dataset_alpha, dataset_merged, hard_mode = False)
...
class DeepMatting(nn.Module):
	...
	def forward(self, x, batch_trimapsT):
		...
		return pred_trimap, trimap_loss
...
args['cuda'] = True
args['resume'] = 'saved_models_FG/model_best.pth.tar'
args['seed'] = 1
# cuda
...
def save_checkpoint(state, is_best, filename='saved_models_FG/checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'saved_models_FG/model_best.pth.tar')
	...
	for epoch in range(initial_epoch, max_epochs):
		...
		for batch, sample_batched in enumerate(dataloader):
			...
			if(batch % print_freq == 0 and not batch==0):
				...
				#print(train_loss)
				plt.plot(train_loss)
				plt.savefig('test_predict/train_loss_fg.png')
				total_loss = 0  
...
def test():
    ...
    for i in range(1000):
        ...
        test_fg = Variable(torch.Tensor(np.expand_dims(fg,0).astype(np.float64)).permute(0,3,1,2), volatile=True).cuda()

...
def load_path(alpha, eps, tri, hard_mode = False):
	images_alpha = sorted(os.listdir(alpha))
	images_merged = sorted(os.listdir(eps))
	images_trimaps = sorted(os.listdir(tri))
	m_ix = [images_merged.index('_'.join(i.split('_')[:-1])+'.png') for i in images_trimaps]
	images_merged_1 = list(np.array(images_merged)[m_ix])
	a_ix = [images_alpha.index('_'.join(i.split('_')[:-1])+'.jpg') for i in images_merged_1]
	images_alpha_1 = list(np.array(images_alpha)[a_ix])
	alphas_abspath = [os.path.join(alpha,common_path) for common_path in images_alpha_1]
	epses_abspath = images_merged
	epses_abspath = [os.path.join(eps,common_path) for common_path in images_merged_1]
	trimaps_abspath = [os.path.join(tri, common_path) for common_path in images_trimaps]
	return np.array(alphas_abspath), np.array(epses_abspath), np.array(trimaps_abspath)

def load_data(batch_alpha_paths, batch_eps_paths, batch_trimap_paths):
	...
	while batch < batch_size:
		...
		trimap = misc.imread(batch_trimap_paths[i]).astype(np.float32)
		eps = misc.imread(batch_eps_paths[i]).astype(np.float32)
		batch_i  = preprocessing_single(alpha, eps, trimap, batch_alpha_paths[i])	
...
def preprocessing_single(alpha, eps, trimap, name,image_size=320):
	alpha = np.expand_dims(alpha,2)
	crop_size = random.choice([320,480,620])
	flip = random.choice([0,1])
	trimap_proxy = np.copy(alpha)
	trimap_proxy = generate_trimap(trimap_proxy, alpha)
	i_UR_center = UR_center(trimap_proxy)
	trimap = np.round(np.expand_dims(trimap, -1)/128.0)
...
class MattingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        batch_trimap_paths = paths_trimaps[batch*batch_size:(batch+1)*batch_size]
        #print(len(batch_alpha_paths) ,len(batch_merged_paths) ) 
        batch_RGBs, batch_trimaps, batch_alphas = load_data(batch_alpha_paths, batch_merged_paths, batch_trimap_paths)
        #print('inside dataloader')
        batch_RGBsT, batch_trimapsT, batch_alphasT = [(torch.Tensor(batch_RGBs.astype(np.float64))),(torch.LongTensor(batch_trimaps.astype(np.int64))),(torch.Tensor(batch_alphas.astype(np.float64)))]

        batch_RGBsT, batch_trimapsT, batch_alphasT = [batch_RGBsT.permute(0,3,1,2), batch_trimapsT.permute(0,3,1,2), batch_alphasT.permute(0,3,1,2)]
        return {'batch_RGBsT':torch.squeeze(batch_RGBsT,0), 'batch_trimapsT':torch.squeeze(batch_trimapsT,0), 'batch_alphasT':torch.squeeze(batch_alphasT,0)}
...
def load_alphamatting_path(test_alpha):
    ...
    return images, tri_images, sorted(os.listdir(trimap_path)) 


def load_alphamatting_data(rgb_path, trimap_path ):
    ...
    return np.array(rgb), np.array(trimap), all_shape, trimap_size
...
dataset_trimap = 'Training_set/trimap_new'

paths_alpha, paths_merged, paths_trimaps = load_path(dataset_alpha, dataset_merged, dataset_trimap, hard_mode = False)
...
class DeepMatting(nn.Module):
	...
	def forward(self, x, batch_trimapsT):
		...
		_,pred_trimaps = torch.max(x11d, 2) 	
...
args = {}
args['cuda'] = True
args['resume'] = 'saved_models_fixed/model_best.pth.tar'
args['seed'] = 1
...
def save_checkpoint(state, is_best, filename='saved_models_fixed/checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'saved_models_fixed/model_best.pth.tar')
...
def train():
	...
	for epoch in range(initial_epoch, max_epochs):
		...
		for batch, sample_batched in enumerate(dataloader):
			#print('entered loop')
			batch_RGBsT, batch_trimapsT, batch_alphasT = Variable(sample_batched['batch_RGBsT']), Variable(sample_batched['batch_trimapsT']),Variable(sample_batched['batch_alphasT'])
			#print('inside epoch', epoch)
			if USE_CUDA:
				batch_RGBsT, batch_trimapsT, batch_alphasT = [batch_RGBsT.cuda(), batch_trimapsT.cuda(), batch_alphasT.cuda()]
			optimizer.zero_grad()
			b_input = batch_RGBsT
			#print(np.unique(batch_trimapsT.data.cpu().numpy()))
			...
			if(batch % print_freq == 0 and not batch==0):
				...
				#print(train_loss)
				plt.plot(train_loss)
				plt.savefig('test_predict/train_loss_fixed.png')
				total_loss = 0  
...
def test():
    ...
    for i in range(1000):
        #print('inside test')   
        RGB, trimap, shape_i, trimap_size = load_alphamatting_data(test_RGBs[i], test_trimaps[i])
        print(collections.Counter(list(trimap.flatten())))
        test_RGB = Variable(torch.Tensor(np.expand_dims(RGB,0).astype(np.float64)).permute(0,3,1,2), volatile=True).cuda()
        test_trimap = Variable(torch.LongTensor(np.expand_dims(trimap,0).astype(np.int64)).permute(0,3,1,2), volatile=True).cuda()
        b_input = test_RGB
        # predictions

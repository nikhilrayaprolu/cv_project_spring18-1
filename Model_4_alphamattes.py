...
def load_data(batch_alpha_paths, batch_eps_paths, batch_BG_paths):
    ...
    while batch < batch_size:
        ...
        batch_i,raw_RGB = preprocessing_single(alpha, BG, eps, batch_eps_paths[i])	
...
def preprocessing_single(alpha, BG, eps, name, image_size=320):
    ...
    trimap = misc.imread('gen_trimap/'+ name.split('/')[-1]).astype(np.float32)
    trimap = np.expand_dims(misc.imresize(trimap, alpha.shape[:2], interp = 'nearest',mode='L'), -1)
    trimap_proxy = np.copy(alpha)
    trimap_proxy = generate_trimap(trimap_proxy, alpha)
    if 128 not in np.unique(trimap):
        trimap = trimap_proxy
...
class DeepMatting(nn.Module):
    ...
    def load_my_state_dict(self, model_dict):
        ...
        model_p = 0
        for count, name in enumerate(list(model_state.keys())):
...
def test():
    ...
    for i in range(1000):
        ...
        # predictions
        test_out, loss = model(b_input, test_alpha, test_trimap)
